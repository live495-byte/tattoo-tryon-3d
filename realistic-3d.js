...
function smoothAndPrepareMesh(mesh) {
  ...
  const isEye = meshName.includes('eye') || meshName.includes('eyeball');

  try {
    // V10 already contains correct smooth normals from the original Blender asset.
    // Do not overwrite them — especially important for face, ears and fingers.
    ...
    console.warn('Normal preparation skipped:', e);
  }
  ...
  sourceMaterials.filter(Boolean).forEach(material => {
    ...
    if (isEye) {
      ...
      if (material.color && !material.map) {
        material.color.set(0xd8d4cc);
      }
    } else {
      material.roughness = .82;
      if (material.color && !material.map) {
        material.color.set(0xd39b7c);
      }
    }
...
function modelCandidates(gender) {
  // Версии файлов специально различаются, чтобы браузер/GitHub Pages
  // не подсовывали старую 3D-модель из кэша.
  if (gender === 'male') {
    return [
      `models/male-v10.glb?v=20260902-10`,
      `models/male-v8.glb?v=20260902-10`,
      `models/male-v2.glb?v=20260902-10`,
      `models/male.glb?v=20260902-10`
    ];
  }

  return [
    `models/female-v2.glb?v=20260902-10`,
    `models/female.glb?v=20260902-10`
  ];
}
...
async function loadModel(gender) {
  ...
  try {
    ...
    const loadedName = result.url.split('?')[0].split('/').pop();
    console.info('[Tattoo Try-On] Loaded model:', result.url);

    toast(
      gender === 'male'
        ? `Мужская модель готова: ${loadedName}`
        : `Женская модель готова: ${loadedName}`
    );
  } catch (error) {
    console.error(error);
